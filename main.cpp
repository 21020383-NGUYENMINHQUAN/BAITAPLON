...
// tao cua so game va render doi tuong
...
void main_program(RenderWindow window)
{
    ...
    while (GameRunning)
    {
        ...
        float running_time = SDL_GetTicks();
        running_time *=0.001;
        //cout<<running_time<<"\n";
        ...
        if ((keystates[SDL_SCANCODE_SPACE] || keystates[SDL_SCANCODE_UP]) && Plane.GetY()>=0)
            ...
            if (keystates[SDL_SCANCODE_LEFT])
                {Plane.setCoordinates(Plane.GetX()-5, Plane.GetY());SDL_Delay(1);}
        else
            ...
            if (keystates[SDL_SCANCODE_DOWN])
                {Plane.setCoordinates(Plane.GetX(), Plane.GetY()+5);SDL_Delay(1);}
        else if (running_time>=2) Plane.setCoordinates(Plane.GetX(), Plane.GetY()+2);
